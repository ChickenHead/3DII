...
void PolyhedronUtils::segmentAroundAFacet(std::vector<Polyhedron::Facet_handle> & facet_group, const Polyhedron::Halfedge_around_facet_circulator facet_circulator, double sensitivity, vxfloat3 average_normal, int call_depth)
{
	
	Polyhedron::Facet_handle center_facet_handle = facet_circulator->facet();
    average_normal.x = (average_normal.x*facet_group.size() + center_facet_handle->normal().x())/(facet_group.size()+1);        
	average_normal.y = (average_normal.y*facet_group.size() + center_facet_handle->normal().y())/(facet_group.size()+1);        
	average_normal.z = (average_normal.z*facet_group.size() + center_facet_handle->normal().z())/(facet_group.size()+1);        
	...
	for ( i=0; i < facet_circulator->facet_degree() ; currEdge = currEdge->next(), i++){
		...
		if (!currEdge->opposite()->facet_begin()->facet()->is_visited()) {
			...
			/*if ((center_facet_handle->normal() * currEdge->opposite()->facet_begin()->facet()->normal()) > sensitivity) {
				segmentAroundAFacet(facet_group, currEdge->opposite()->facet_begin(), sensitivity, call_depth+1);
			} */
			...
			vxfloat3 curr_normal = vxfloat3(x1, y1, z1);
			float inner_product = average_normal.x*curr_normal.x + average_normal.y*curr_normal.y + average_normal.z*curr_normal.z; 


			if (inner_product > sensitivity) {
				segmentAroundAFacet(facet_group, currEdge->opposite()->facet_begin(), sensitivity, average_normal, call_depth+1);
			}
