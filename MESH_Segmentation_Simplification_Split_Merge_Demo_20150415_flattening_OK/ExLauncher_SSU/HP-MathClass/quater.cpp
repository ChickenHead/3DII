...
vectorM Quater2EulerAngle( quater const& q )
{
	return Matrix2EulerAngle( Quater2Matrix( q ) );
}

quater EulerAngle2Quater( vectorM const& v )
{
...
quater pow(vectorM const& w, m_real a)
{
    return exp(a * w);
}

vectorM ln(quater const& q)
{
    ...
    return vectorM(sc * q.p[1], sc * q.p[2], sc * q.p[3]);
}
...
vectorM rotate(quater const& a, vectorM const& v)
{
    quater c = a * quater(0, v.x(), v.y(), v.z()) * inverse(a);
    return vectorM(c.x(), c.y(), c.z());
}
