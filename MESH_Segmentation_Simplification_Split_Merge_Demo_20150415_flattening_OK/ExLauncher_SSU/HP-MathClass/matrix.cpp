...
{
    vectorM v1( s, 0.0, 0.0 );
    vectorM v2( 0.0, s, 0.0 );
    vectorM v3( 0.0, 0.0, s );
    return matrix( v1, v2, v3 );
}

matrix scaling( m_real sx, m_real sy, m_real sz )
{
    vectorM v1( sx, 0.0, 0.0 );
    vectorM v2( 0.0, sy, 0.0 );
    vectorM v3( 0.0, 0.0, sz );
    return matrix( v1, v2, v3 );
}

matrix rotation( m_real angle, vectorM const& axis )
{
    vectorM v = angle * normalize( axis );
    quater q = exp( v/2.0 );
    return Quater2Matrix( q );
}

matrix reflection( vectorM const& normal )
{
...
matrix mexp( vectorM const& v )
{
	m_real t = v.length();
	vectorM u = normalize( v );
	...
	vectorM v1( x*x*k + c, x*y*k - z*s, x*z*k + y*s );
	vectorM v2( x*y*k + z*s, y*y*k + c, y*z*k - x*s );
	vectorM v3( x*z*k - y*s, y*z*k + x*s, z*z*k + c );

	return matrix( v1, v2, v3 );
}

vectorM mlog( matrix const& m )
{
	...
	return vectorM( m.getValue(2,1)-m.getValue(1,2),
...
Matrix2EulerAngle( const matrix& m )
{
    ...
    return vectorM(x,y,z);
}

matrix
EulerAngle2Matrix( const vectorM& v )
{
